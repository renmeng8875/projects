...
<form id="fm_es" method="post" action="${ctx}/StationLetter/editLetter.do">
<input type="hidden" name="csrfToken" value="${csrfToken}"/>
<input name="letterid" type="hidden" value="${letter.letterid}" />
<div class="pad10">
  <div class="adm-text ul-float">
      ...
      <li>
        ...
        <input class="l mr20" name="letter" id="letter"  type="text" value="${letter.letter}" datatype="spchar,*2-36"  nullmsg="请输入标题!" errormsg="标题长度必须在2~36位字符范围内!" />
        <span>&nbsp;</span>
      </li>
      <li>
      	<span class="need" style="float:left;">*</span>
      	<span class="mr0 m" style="float:left;">置顶：</span>
        <input type="radio" value="0" name="isTop" <c:if test="${letter.top == null || letter.top == 0 }"> checked="checked"</c:if> />否
        <input type="radio" value="1" name="isTop" <c:if test="${letter.top == 1 }"> checked="checked"</c:if> />是
        <span>&nbsp;</span>
      </li>
      <li>
      	<span class="need" style="float:left;">*</span>
        ...
        <textarea name="content" id="content" cols="50" rows="21" datatype="spchar,*1-1024" errormsg="内容长度必须为1-1024个字符！"   nullmsg="请输入内容!">${letter.content}</textarea><span id="contentTip"></span>
      </li>
    </ul>
  </div>
</div>
