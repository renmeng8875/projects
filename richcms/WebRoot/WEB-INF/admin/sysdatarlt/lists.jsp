...
<script type="text/javascript">
		...
		gridManager = $("#maingrid").ligerGrid({
			columns : [ {
				...
				render:function(item){
					 return  '<a class="treeopra" href="${ctx}/Sysdatarlt/edit.do?srcid='+item.srcid+'&type='+item.datatype+'">修改</a>';
				}
			}
  ...
  function add() {
		window.location.href = "${ctx}/Sysdatarlt/add.do";
  }
  function del(){
		...
		if (data.length == 0)
			...
			$.ligerDialog.confirm('确定删除' + checkedIds.join(',') + '?',function(btn){
			   ...
			   $.ajax({
					   type: "post",
					   url: "${ctx}/Sysdatarlt/del.do?csrfToken=${csrfToken}",
