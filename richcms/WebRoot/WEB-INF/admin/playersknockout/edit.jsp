					...
					$.ajax({
					   ...
					   success: function(data){
					   		...
					   		if(styleList && styleList.length>0) {
	                             ...
	                             loadTpl('${pk.style}'==''?styleList[0].styleid:styleId);
					   		}else{
					   			$("#mystyle").empty();
					   			$("#mystyle").append("<option styleid='' value='' selected>--请选择--</option>");
					   		} 
...
$(function(){
	...
	if(logo!='')
	{
		try{
			...
			$("#container1").append('<div class="display-image" attr="displayImage"  dname="'+logoPath+'" title="点击显示原图"><a style="position: absolute;" href="<c:url value="/upload/"/>'+logoPath+'" target="blank"><img width="120px" height="120px" src="<c:url value="/upload/"/>'+logoPath+'"/></a><div style="position: absolute; margin-left:104px;display:none;cursor:hand;" class="l-icon l-icon-delete"></div></div>');
	...
	if(imgs!=''){
		try{
			var imgJson = JSON2.parse(imgs);
			var path=storageUrl+(imgJson[1]?imgJson[1].path:'');
			var imagePath=(imgJson[1]?imgJson[1].path:'');
			$("#container2").append('<div class="display-image" attr="displayImage" dname="'+imagePath+'" title="点击显示原图"><a style="position: absolute;" href="<c:url value="/upload/"/>'+imagePath+'" target="blank"><img width="120px" height="120px" src="<c:url value="/upload/"/>'+imagePath+'"/></a><div style="position: absolute; margin-left:104px;display:none;cursor:hand;" class="l-icon l-icon-delete"></div></div>');
	        $("#container2").append('<input type="hidden" name="jsonimg" value="'+imagePath+'" />');
        }catch(e){
        }	
	}
	...
	var activitiesLogo = '${pk.activitiesLogo}';
	if(activitiesLogo!='')
	{	
		$("#container3").append('<div class="display-image" attr="displayImage" dname="'+activitiesLogo+'" title="点击显示原图"><a style="position: absolute;" href="<c:url value="/upload/"/>'+activitiesLogo+'" target="blank"><img width="120px" height="120px" src="<c:url value="/upload/"/>'+activitiesLogo+'"/></a><div style="position: absolute; margin-left:104px;display:none;cursor:hand;" class="l-icon l-icon-delete"></div></div>');
		$("#container3").append('<input type="hidden" name="jsonimg" value="'+activitiesLogo+'" />');
	}
...
// 		inputObj.remove();
	...
	if($("#forwardUrl").val()!='')
	{
		$("input[name='temporjump'][value=0]").attr("checked",true);
		$("#templateset2").show();
		$("#templateset22").show();
		$("#templateset1").hide();
		$("#templateset11").hide();
...
<div class="pad10">
  <div class="adm-text ul-float">
    <ul class="clearfix">
      <li><span class="need" >*</span><span class="mr20 m">玩家争霸名称：</span>
      ...
      <li><span class="need" >*</span><span class="mr20 m">英文名称：</span>
        <input style="width:250px;" class="l mr20" name="pk" id="pk" type="text" value="${pk.pk}" datatype="zhs2-50" sucmsg="验证通过,名称可用!" nullmsg="英文名称不能为空!" errormsg="英文名称必须在2~50位数字和小写字母组合的范围内,且第一位必为小写字母!"   ignore="ignore"  ajaxurl="${ctx }/pk/checkName.do?csrfToken=${csrfToken}&pkid=${pk.pkid}"  />
      </li>
      ...
      <li><span class="mr20 m">显示/隐藏：</span>
        <input name="status"  type="radio"  value="1" <c:if test="${pk.status==1}">checked="checked"</c:if> />显示   &nbsp;&nbsp;&nbsp;&nbsp;
        <input name="status"  type="radio"  value="0"  <c:if test="${pk.status==0}">checked="checked"</c:if>/>隐藏
      </li>
     <li><span class="need" >*</span><span class="mr20 m">seo标题&nbsp;&nbsp;&nbsp;：</span>
      ...
      <li><span class="need" style="position: absolute;">*</span><span class="mr20 m" style="margin-left: 6px;">seo描述&nbsp;&nbsp;&nbsp;：</span>
       ...
       <li>
		<span class="mr20 m"><input type="radio" name="temporjump" id="temporjump1" value="1" checked="checked" />显示模板设置&nbsp;&nbsp;</span>
		<span class="mr20 m"><input type="radio" name="temporjump" id="temporjump2" value="0" />显示跳转设置</span>
	  </li>
		<li id="templateset1">
	  ...
	  <li id="templateset22">
        ...
        <span><input name="isForward"  type="radio"  value="1"  <c:if test="${pk.isForward==1}">checked="checked"</c:if>  />是   &nbsp;&nbsp;&nbsp;&nbsp;</span>
        <span><input name="isForward"  type="radio"  value="0" <c:if test="${pk.isForward==0}">checked="checked"</c:if> />否</span>
      </li>
      <li id="templateset2">
      	<span class="need" >*</span><span class="mr20 m">跳转地址：</span>
        <span><input style="width:250px;" class="l mr20" name="forwardUrl" id="forwardUrl" type="text"  value="${pk.forwardUrl}" datatype="url" ignore="ignore"  sucmsg="验证通过!"  errormsg="跳转地址不是有效的url"/></span>
