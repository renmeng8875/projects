...
$(function(){
	...
	var selectObj = $("#selectCatid").get(0);
	var ops = selectObj.options;
	var catid = ${travelset.catid};
	for(var i=0;i<ops.length;i++)
	{
		if(ops[i].value==catid){
			ops[i].selected = true;
			break;
		}	
	}
	try{
		var imgs = '${travelset.img}';
		if(imgs!=''){
			var imgs = JSON2.parse(imgs);
		}
		for(var key in imgs)
		{
			var imgName = imgs[key];
			$("#imageContainer").append('<div class="display-image" attr="displayImage" dname="'+imgName+'" title="点击显示原图"><a style="position: absolute;" href="<c:url value="/upload/"/>'+imgName+'" target="blank"><img width="97px" height="97px" src="<c:url value="/upload/"/>'+imgName+'"/></a><div style="position: absolute; margin-left:82px;display:none;cursor:hand;" class="l-icon l-icon-delete"></div></div>');
	        $("#imageContainer").append('<input type="hidden" name="jsonimg" value="'+imgName+'" />');	
		}
	}catch(e){
...
<body style=" padding: 4px;background-color: #fff;" >
	<h2 class="adm-title"><a class="title">MM之旅修改</a><a class="fr btn" href="javascript:history.go(-1);">+返回</a></h2>
<form id="fm_es" method="post" action="${ctx}/TravelSet/edittravelset.do" >
<input type="hidden" name="csrfToken" value="${csrfToken}"/>
<input name="travelid" value="${travelset.travelid}" type="hidden"/>
<div class="pad10">
    ...
    <ul class="clearfix"> 
      ...
      <span class="need" style="float:left;">*</span>
       <span style="width:280px;">MM之旅中文标识：</span>
        <input type="text" name="travelname" id="travelname" value="${travelset.travelname}" datatype="*2-50" />
      </li>
      <li>
      <span class="need" style="float:left;">*</span>
      <span style="width:280px;">MM之旅英文标识：</span>
        <input type="text" name="travel" id="travel" value="${travelset.travel}" datatype="*2-50"/>
      </li>      
      <li><span >标签类:</span>
        <select id="tagtype" name="tagtype">
        	<option value="os" <c:if test="${travelset.tagtype=='os'}"> selected="selected"</c:if> >手机系统</option>   
        	<option value="tag" <c:if test="${travelset.tagtype=='tag'}"> selected="selected"</c:if> >身份标签</option>
        	<option value="like" <c:if test="${travelset.tagtype=='like'}"> selected="selected"</c:if> >个人爱好</option>                        
        </select>
...
<input type="submit" id="submit" value="修&nbsp;&nbsp;改" /></div>
</form>
</body>
</html>