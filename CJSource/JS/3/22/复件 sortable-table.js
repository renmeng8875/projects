(function () {
	...
	function SortableTable(table) {
		this.table=table;
		this.headers=table.tHead.rows[0].cells;
	...
	SortableTable.prototype={
		sortCol:function (colIndex,dir) {
			...
			for (i=0;i<this.headers.length;i++) {
				img=getElementsByClassName("sortDirImage",this.headers[i])[0];
				if (img)
					img.parentNode.removeChild(img);
			}
			img=getElementsByClassName("sortDirImage",this.headers[colIndex])[0];
			if (!img) {
				this.headers[colIndex].innerHTML+='<img class="sortDirImage" src="images/desc.gif" />';
				img=getElementsByClassName("sortDirImage",this.headers[colIndex])[0];
			}
			if (dir==1)
				img.src="images/asc.gif";
			else img.src="images/desc.gif";
		},
	...
	addEvent(window,"load",function () {
		new SortableTable(getElementsByClassName("sortableTable")[0]).sortCol(9,-1);
	});
