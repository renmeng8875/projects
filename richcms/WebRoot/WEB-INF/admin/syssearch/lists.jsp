...
<script type="text/javascript">
	...
	function itemclick(item) {
		if (item.id) {
			switch (item.id) {
			case "1":
				window.location.href = "${ctx}/Syssearch/add.do;" ;
				break;
			case "2":
				...
				window.location.href = "${ctx}/Syssearch/add.do?sid="+row.sid;
				return;
			case "3":
				...
				if (data.length == 0)
					...
					$.ligerDialog.confirm('确定删除' + checkedIds.join(',') + '?',function(btn){
					   ...
					   $.ajax({
							   type: "post",
							   url: "${ctx}/Syssearch/del.do?csrfToken=${csrfToken}",
...
<body style="overflow-x: hidden; padding: 4px;">
	<h2 class="adm-title clearfix"><a class="fl title">搜索配置管理</a><a href="${ctx}/Syssearch/add.do" class="fr btn">+添加配置</a></h2>
