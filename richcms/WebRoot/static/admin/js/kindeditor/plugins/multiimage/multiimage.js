...
K.extend(KSWFUpload, {
	init : function(options) {
		...
		self.div = K(options.container).html([
			...
			'</div>',
			'<div class="ke-inline-block ke-swfupload-desc">' + options.isAddwaterMark + '&nbsp;&nbsp;<input type="checkbox" name="isAddWaterMask" /></div>',
		...
		K('.ke-swfupload-startupload input', self.div).click(function() {
			var url = self.swfu.getSetting("upload_url");
			var url = url.replace("&isAddWaterMask=0","").replace("&isAddWaterMask=0","");
			if($("input[type='checkbox']").attr("checked")=="checked")
			{
				url = url+"&isAddWaterMask=1";
			}
			else
			{
				url = url+"&isAddWaterMask=0";
			}	
			self.swfu.setUploadURL(url);	
...
KindEditor.plugin('multiimage', function(K) {
	...
	self.plugin.multiImageDialog = function(options) {
		...
		var swfupload = K.swfupload({
			...
			isAddwaterMark : lang.isAddwaterMark,
