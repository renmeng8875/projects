...
<script type="text/javascript">
  ...
  <body>
    <h2 class="adm-title clearfix">
    	<a class="fl title">渠道访问数据统计</a>
    </h2>
	<div>
		<form id="form1" method="post" action="${ctx}/Stataccess/channelLists.do">
		...
		<form id="form2" method="post"  action="${ctx}/Stataccess/exportChanelExcel.do">
			<input type="hidden" name="csrfToken" value="<%=TokenMananger.getTokenFromSession(session)%>"/>
			<input type="hidden" name="ctone" id="ctone" value="${ctone}"/>
    ...
    <tfoot>
    	<tr>
    		<td colspan="<c:choose><c:when test="${cttwo eq 'singledata'}">14</c:when><c:otherwise>12</c:otherwise></c:choose>">
	    		<input type="hidden" id="url" value="${ctx}/Stataccess/channelLists.do?csrfToken=${csrfToken}&&beginTime=${beginTime}&&endTime=${endTime}">
