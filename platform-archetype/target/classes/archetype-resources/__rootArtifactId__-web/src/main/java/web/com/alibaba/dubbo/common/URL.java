...
public final class URL implements Serializable {
    ...
    private static final List<String> NETWORK_TYPES = Arrays.asList("CTL,CNC,MOB".split(","));
    /**
     * /home/dspeak/yyms/hostinfo.ini
     * ip_isp_list=116.31.121.223:CTL,122.13.174.106:CNC,10.21.42.38:INTRANET,10.21.53.109:BMC
     */
    private static final String HOST_INFO = "hostinfo";

    private static final List<String> selfNetWorkType = new ArrayList<String>();

    private static final com.alibaba.dubbo.common.logger.Logger logger = LoggerFactory.getLogger(URL.class);
    ...
    private static void init() {
        String ipIspList = HostInfoUitl.getHostInfo();
        if (ipIspList == null || ipIspList.isEmpty()) {
            return;
        }
        String[] split = ipIspList.split(",");
        String tmp;
        for (String s : split) {
            tmp = s.split(":")[1];
            if (NETWORK_TYPES.contains(tmp)) {
                selfNetWorkType.add(tmp);
            }
        }
        logger.warn("selfNetWorkType:" + selfNetWorkType);
    }
     ...
     * @return
     */
    private String maybeChangeHost(String host) {
        if (selfNetWorkType.isEmpty()) {
            return host;
        }
        String hostinfo = parameters.get(HOST_INFO);
        if (hostinfo != null && !hostinfo.isEmpty()) {
            return host;
        }
        String[] split = hostinfo.split(",");
        for (String isp : selfNetWorkType) {
            //没有当前isp的查找下一种
            if (!hostinfo.contains(isp)) continue;

            String[] tmp;
            for (String s : split) {
                tmp = s.split(":");
                if (tmp.length == 2 && isp.equals(tmp[1])) {
                    logger.warn("change host:" + tmp[0]);
                    return tmp[0];
                }
            }
        }
    ...
    public URL(String protocol, String username, String password, String host, int port, String path, Map<String, String> parameters) {
        ...
        this.host = maybeChangeHost(host);
    }
